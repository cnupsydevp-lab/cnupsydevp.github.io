  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      /* fetch 자체가 실패하면(네트워크 끊김·CORS 막힘) 브라우저가 "Failed to fetch"
         ...
         [2026-09-12] "잠시 후 다시 시도해 주세요" 로 끝내지 않는다. 보내는 곳이
         다른 출처의 서버라 그쪽이 내려가 있으면 몇 번을 눌러도 계속 실패하는데,
         그 경우 방문자에게 남는 길은 이메일뿐이다. 알림창은 6초 뒤 사라지므로
         주소를 여기 적어도 놓치기 쉽다 — 그래서 폼 아래에 늘 떠 있는 줄
         (.form-fallback)을 가리킨다. 여기서 주소를 직접 읽지 않는 이유는 두 가지다:
         페이지마다 주소가 다르고(대학원=지도교수, 인턴=연구실), 이 상자는
         textContent 로 글자만 넣어서 눌러지는 링크를 만들 수 없다. */
      showToast((err.fromApi && err.message)
                || '전송에 실패했습니다. 잠시 후 다시 시도하시거나, 폼 아래 이메일 주소로 보내 주세요.', true);
    } finally {
      btn.disabled = false;
    }
  });
