...
그래서 같은 그림을 세 형식으로 함께 만든다:
    ...
    favicon-v5.svg           손으로 관리 (이 스크립트가 건드리지 않음) — 크롬·파이어폭스
    favicon-v5.ico           16·32·48 세 크기를 한 파일에 — 사파리·구형 브라우저
    apple-touch-icon-v5.png  180x180 — iOS 홈 화면, 맥 사파리 대체용

파일명 끝의 -v5 는 캐시를 끊기 위한 것이다. 아래 두 번째 경고를 보라.
(2026-09-12: 그림은 그대로 두고 캐시만 끊으려고 v4 → v5 로 올렸다.)
...
⚠️ 색이나 글자를 바꾸면 favicon-v5.svg 도 같이 손으로 고쳐야 한다.
   그 한 장만 따로 놀면 크롬과 사파리가 서로 다른 아이콘을 보여 준다.

⚠️ 그림을 바꿀 때는 ?v= 쿼리를 올리지 말고 파일명의 -v5 를 -v6 으로 올린다.
   ...
   일괄 수정 (og-image-v4 까지 같이 바뀌지 않게 접두사를 붙여 지정한다):
       sed -i -e 's|favicon-v5|favicon-v6|g' \
              -e 's|apple-touch-icon-v5|apple-touch-icon-v6|g' *.html
